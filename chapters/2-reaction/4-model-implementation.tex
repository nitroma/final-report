...
A co-current coolant flow within the secondary cooling tube serves to remove excess heat from the reaction. A nonisothermal flow coupling links heat and mass transfer.

\paragraph{Mass transfer: $k$-$\varepsilon$ Turbulence model}

Due to the high flow rate through the tube and its small diameter, the flow regime is turbulent. Temperature-dependent fluid density and viscosity are determined from the COMSOL material library. The inlet boundary condition is set to a mass flow of \SI{0.3}{\kg\per\s}
