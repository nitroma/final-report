...
H-Mordenite catalyst particles are used in the reaction. From the rate equation, it can be assumed that all of the nitration reaction takes place in the solid-fluid interface. Hence, it is important to understand the effect of external and internal diffusional limitation of the catalys

The effective diffusional coefficient $D_e$ is calculated to be 4.42 \times 10\textsuperscript{-10} using the widely-used equation from [ref]: 
\begin{equation}
...
The effective diffusional coefficient is used to calculate Thiele modulus $\phi$, which is needed to calculate the effectiveness factor $\eta$. For the ease of modelling, the pellets are assumed to be perfectly spherical, and therefore the effectiveness factor is calculated as: 
\begin{equation}
