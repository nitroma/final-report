...
\begin{equation}
    T_{max} = T_s + \frac{S}{4k}R^2
    \label{eqn:radialheattransfer}
\end{equation}
where temperature at the wall ($T_s$) at 330K. 

The maximum temperature ($T_{max}$) was calculated to be 387K, which denotes a temperature change ($\Delta T$) of 57K. This temperature was deemed to be risky as it is within a close (refer safety) distance to a thermal runaway at 413K, according to Chen et al. At 413K, the decomposition of nitric acid will occur and it releases nitrogen dioxide ($NO_2$), which is unfavourable for this reaction because the
