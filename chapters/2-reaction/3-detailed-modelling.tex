...
A final number of tubes ($n_t$) of 19 was chosen based on the centered hexagonal number rule which has practical usage in industry. 
\begin{equation}
    n^3 
\end{equation}
From plot Y, it can be observed that a final number of tubes ($n_t$) of 19 in the multitubular packed-bed reactor will provide a required length ($L$) of 4.2m, and allowing an acceptable pressure drop ($\Delta P$)of 0.1bar.  
