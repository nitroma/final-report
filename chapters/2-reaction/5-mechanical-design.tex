...
The inlet and outlet of the reactant flow were placed at the top right and top left of the shell dome respectively. Both ports were estimated to have a acceptable feed liquid velocity range (U) of 0.1 - 1 $ms^{-1}$. The cross sectional area (A) is calculated by dividing the volumetric flowrate ($Q_v$) shown in the equation \ref{eqn:area} below. Then, the outer pipe diameter (OD) was calculated using the equation \ref{eqn:portOD} below. 
\begin{align}
    A &= \frac{Q_v}{U}
    \label{eqn:area} &
