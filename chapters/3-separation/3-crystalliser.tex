...
The diameter of the stirrer, $D_r$, has been designed to be just touching the vessel wall. Details of this will be elaborated in Section \ref{sec:heat transfer}. In the end, Equations \ref{eq:amount solid A equilibrium} to \ref{eq:crystalliser volume} form a system of algebraic equations and can be solved simultaneously for each value of $V$ to compute the mass flow rate of solid p-NT, $\dot{M}_{\mathrm{liquid}}$, that will form. A crystalliser recovery, $r_c$, can be defined where
