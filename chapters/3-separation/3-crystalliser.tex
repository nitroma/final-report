...
\noindent where $B$ is the secondary nucleation rate or birth rate in m$^{-3}$ s$^{-1}$, $K_B$ is the rate law constant, $\rho_m$ is the slurry concentration or magma density that would be given in Equation below, and $N$ is the term that gives measure of the intensity of agitation in the system such as the speed of rotation of the stirrer. The exponents, $j$, $l$, and $b$ vary depending on the system.
...
To describe the kinetics of crystal growth, a population balance must be performed. The importance of the population balance is widely acknowledged and has been the central focus of the works of Randolph and Larson. \cite{richardson_chemical_nodate} \cite{randolph_theory_1971} The population balance, together with the mass balance, are able to specify the process of crystal growth. For a continuous MSMPR crystalliser operated under steady state with no crystals present in the feed, the assumptions are the following
...
\noindent where $L$ is the crystal size, $n$ is the population density of crystals defined as the number of crystals per unit size, $n_0$ is the population density of nuclei, $G$ is the growth rate defined as 

\begin{equation}
    G = \frac{dL}{dt}
\end{equation}

\noindent and $\tau$ is the residence time of the crystalliser defined as 
