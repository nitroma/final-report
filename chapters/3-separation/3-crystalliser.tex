...
\noindent These assumptions come from what is commonly referred to as McCabe's $\Delta L$ law. The steady state CSD derived from the population balance over the system is:

\begin{equation}
    n = n_0 ~exp(-\frac{-L}{G\tau})
\end{equation}

\noindent where $L$ is the crystal size, $n$ is the population density of crystals defined as the number of crystals per unit size, and $n_0$ is the population density of nuclei. $G$ is the growth rate defined as 
...
The volumetric flow rate, $Q$, has been approximated to be constant into and out from the crystalliser because the process is for condensed phase operations. \cite{levenspiel_chemical_1999}
