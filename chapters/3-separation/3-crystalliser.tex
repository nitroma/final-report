...
\begin{equation}\label{eq:vantHoff}
    \ln(x) = \frac{H_{f}}{R T}\left(\frac{T}{T_{m}} - 1\right)
\end{equation}
...
\begin{equation} \label{eq:deltaT}
     \Delta T = T - T_{eq}
\end{equation}
whence $T$ is the equilibrium temperature corresponding to concentration $c$ in the liquid, and $T_{eq}$ is the operated temperature to which the system has been super-cooled. Figure \ref{fig:supersaturation} is a schematic which explains these definitions.
...
\begin{equation}
    B = J_{c-c} + J_{c-s}
\end{equation}
 where $J_{c-c}$ is the contribution from crystal-crystal collisions, and $J_{c_s}$ is that from collisions of crystal with solid objects, such as crystal-wall and crystal-stirrer collisions. The forms of $J_{c-c}$ and $J_{c_s}$ are as following,
\begin{equation}
    J_{c-c} = k_{c-c} \varepsilon \rho_m^2 (\Delta C)^b
\end{equation}

\begin{equation}
    J_{c-s} = k_{c-s} \varepsilon \rho_m (\Delta C)^b
\end{equation}

\noindent where $k_{c-c}$ and $k_{c-s}$ are the rate law constants for the individual contributions, and $\varepsilon$ is the quantity corresponding to the $N$ in Equation \ref{eq:secondary nucleation general} such that 
...
\begin{equation}
    D_m = \frac{7.4 \cdot 10^{-8} T_{eq} \sqrt{\alpha M_A}}{\mu_A V_{m,B}^0.6}
\end{equation}
...
The magma density, $\rho_m$, is given as 
...
\begin{equation}
    \dot{M}_{\mathrm{solid}} = \frac{\dot{M}_{\mathrm{total}} (x_in - x)}{1 - x}
\end{equation}

\noindent where $x_in$ is the fraction of p-NT at the inlet of the crystalliser; $x$ is the fraction of p-NT in the liquid in the crystalliser, and due to the MSMPR mode of operation, it is also the fraction of p-NT in the liquid at the outlet of the crystalliser.

$x$ corresponds to $C$, the concentration of p-NT in the liquid as defined in Equation \ref{eq:deltaC}, such that 

\begin{equation}
    \frac{\dot{M}_{\mathrm{liquid}} x}{Q}
\end{equation}
