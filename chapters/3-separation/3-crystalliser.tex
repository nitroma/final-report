...
\begin{equation}
    J_{c-c} = k_{c-c} \varpsilon \rho_m^2 (\Delta C)^b
\end{equation}

\begin{equation}
    J_{c-s} = k_{c-s} \varpsilon \rho_m (\Delta C)^b
\end{equation}

\noindent where $k_{c-c}$ and $k_{c-s}$ are the rate law constants for the individual contributions, and $\epsilon$ is the quantity corresponding to the $N$ in Equation \ref{eq:secondary nucleation general} such that 

\begin{equation}
    \varpsilon = P_0 \frac{\omega_r^3 D_r^5}{V} 
\end{equation}
...
\begin{equation} \label{eq:secondary nucleation final expression}
    B = K_B \Epsilon (\Delta C)^b \rho_m^2
\end{equation}
...
The rate of crystal growth, $G$, is commonly expressed as a function of supersaturation. Data of the rate of crystal growth for the (p-NT + m-NT) system cannot, however, be found in the current literature. Nevertheless, Radhakrishnan and Balakrishnan measured the crystal growth rate for the (m-NT + o-NT) system, which is the most similar and closest to the (p-NT + m-NT) system. \cite{Radhakrishnan} Figure \ref{fig:o-NT + m-NT kinetics} is a plot of the experimentally obtained growth rates against the Stephen number, $Ste$. A power-law correlation has also been fitted for these data and plotted as the red curve in Figure \ref{fig:o-NT + m-NT kinetics},

\begin{equation}
    G = 9.4569 \cdot 10^{-6} \cdot {Ste}^{0.4415}
\end{equation}

\noindent where $G$ is in m/s, and $Ste$ is defined as 

\begin{equation}
    Ste = \frac{C_{P,S} \Delta T}{H_{fusion}}
\end{equation}

whence $C_{P,S}$ is the heat capacity of the solid crystal, $\Delta T$ is the degree of super-cooling as defined in Equation \ref{eq:deltaT}, and $H_{fusion}$ is the heat of fusion like in Equation \ref{eq:vantHoff}. Thus, the correlation directly relates $G$ to $\Delta T$.

\begin{figure}[h]
    \centering
    \includesvg[scale=0.75,inkscapelatex=false]{figures/Kinetics.svg}
    \caption{Crystal growth rate of the (o-NT + m-NT) system.}
