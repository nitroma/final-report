...
\noindent The diameter of the stirrer, $D_r$, has been designed to be just touching the vessel wall to minimise fouling. Details of this will be elaborated in Section \ref{sec:heat transfer crystalliser}.
...
\noindent where $R_1$ is the process side convective heat transfer term, $R_w$ is the conductive heat transfer of the crustalliser wall, $R_2$ is the service side convective heat transfer, and $R_f$ the additional resistance due to fouling. In these calculations, any resistance due to fouling is not taken into account, but the effects are investigated in depth in later sections. %label section with fouling sensitiivty%.
...
where L is the total length of the half pipe, N is the number of revolutions in the helix, C is the circumference of the helix. The system of equations are then solved simultaneously on MATLAB 
...
A suitable value for the service side fouling resistance due to the ethylene glycol solution was found. To look at the process side fouling, the thickness of fouling has been varied to analyse its effect on the heat transfer area and the overall heat transfer coefficient and whether the effect needed to be addressed with any further considerations.:
