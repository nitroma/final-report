...
A crystallisation system consisting of a melt crystalliser (S104) and a subsequent hydraulic wash column (S106) has been chosen as the appropriate separation technique, where PNT is recovered as solid crystals from the crystalliser and separated from the liquid by the hydraulic wash column. A PFD for this system is available in \cref{fig:separator PFD}. The process stream from C101, an air-cooled heat exchanger which cools the bottom flow of distillation S103 becomes the inlet stream for the separation sys
