...
\begin{itemize}
    \item The LL alarm for cooling water flowrate corresponds to a \SI{60}{\percent} drop from the set point.
    \item The pump temperature HH alarm is the methanol boiling up as gas could cause cavitation.
    \item The reactor temperature, gas temperature and liquid temperature HH alarms are \SI{3}{\celsius} below the flash point of o-tol.
    \item The reactor pressure, gas pressure, liquid pressure, recycle pressure and distillation column HH alarms are \SI{2}{\bar} higher than the set point. The gas pressure LL alarm is set at \SI{2}{\bar} lower.
    \item The reactor level HH alarm corresponds to \SI{15}{\percent} of the reactor height.
    \item The LL alarm for the flow to the pump is set at a \SI{90}{\percent} drop from set point.
    \item The HH alarms for the gas flowrate and the recycle flowrate correspond to a \SI{40}{\percent} rise from normal flow.
    \item The HH and LL alarms for the levels in the reflux drum and reboiler are set for a \SI{40}{\percent} rise or drop below designed level.
    \item The temperature HH alarm for the distillation column corresponds to a \SI{20}{\percent} increase compared to the set point.
    \item The HH alarms for the cooling water temperature were designed to sound when the temperature was so high that it could no longer remove, with a the set point flowrate, the same amount of heat that is removed by a \SI{60}{\percent} lower cooling water flowrate at \SI{16}{\celsius}.
\end{itemize}

\subsection{For start-up procedures}

\subsubsection{Manual inspections and testing}
\paragraph{Preliminary inspection of plant sub-section (Step 1)}
    \begin{itemize}
...
\paragraph{Remove shutdown blinds and install running blinds (Step 2)}
    \begin{itemize}
...
\paragraph{Testing of electrical systems (Step 3)}
 \begin{itemize}
...
\paragraph{Tightness testing and bolt tightening (Step 7)}
 \begin{itemize}
...
\paragraph{Purge reactor and column with nitrogen (Step 7)}
 ...
 \begin{equation}
    ...
    \begin{itemize}
        ...
        \item N2 will be flowed at a velocity of \SI{55}{\m\per\s}
        \item A pipe with diameter of one-inch is used
    \end{itemize}
    
\subsubsection{Equipment preparation}

\paragraph{Start-up fan F201 and electric heaters H201 and H203 (Step 6)}
\begin{itemize}
        ...
        \item An start-up efficiency of \SI{60}{\percent} was assumed.
...
\paragraph{Pressurisation and heating of R201 (Step 11)}
\begin{itemize}
...
\begin{itemize}
    \item The volume of the column is 0.31 m$^3$ and it needs to be filled at \SI{15}{\percent}, thus the column of methanol required is 0.0465 m$^3$.
    \item The velocity of methanol is taken as \SI{0.5}{\m\per\s} and the diameter of the pipe as one inch.
...
\paragraph{Steam flow to reboiler H204 (Step 2)}   
    \begin{itemize}
        \item The gas flowrate is reduced by \SI{10}{\percent} its steady state value per minute, so the time to completely stop flow = 10 minutes.
    \end{itemize}
    
\paragraph{Hydrogen and liquid feed to reactor R201 (Step 3)}
    \begin{equation}
...
\paragraph{Cooling water flowrate (Step 11)}
    \begin{itemize}
        \item The cooling water flowrate is reduced by \SI{3}{\percent} its steady state value per minute, so the time to completely stop flow = 33.3 minutes.
    \end{itemize}
    
\subsubsection{Draining and depressurising}   
\paragraph{Depressurise reactor R201 to \SI{1}{\atm} by venting gas (Step 5)}
    \begin{equation}
    ...
    \begin{itemize}
        ...
        \item Assume reactor is \SI{80}{\percent} filled with hydrogen, \SI{20}{\percent} filled with liquid
        \item Assume venting flowrate is twice the steady state outlet flowrate
    \end{itemize}
    
\paragraph{Drain reactor R201 and column S201 (Step 6) (Step 7)}   
    ...
    \begin{itemize}
        \item Assume \SI{7}{\percent} of unit volume is filled with liquid, half the setpoints of the level high-high alarm
        \item A pipe with a one-inch diameter is used (1 inch = 2.54cm)
        \item Velocity of fluid through drain valve is controlled at \SI{1}{\m\per\s}
    \end{itemize}

\subsubsection{Purging with inert gas} 
\paragraph{Purge reactor and column with nitrogen (Step 10)}
...
\paragraph{Replacing running blinds with shutdown blinds (Step 14))}
    \begin{itemize}
        \item There are 22 pipes connected to process units with process fluids.
        \item There is one blind on each process pipe.
        \item The time to replace one blind is estimated at 2 minutes.
...
\paragraph{Inspection of electrical systems and instruments (Step 16)}
    \begin{itemize}
...
\paragraph{Inspection of units, equipment and instruments (Step 17)}
    \begin{itemize}
        \item There are 6 process units with inventory: R201, S201, S204, H204, P201 and F201.
        \item The average time to check each unit is estimated at 10 minutes.
        \item The total time to check would be 6 * 10 = 60 minutes.
...
\paragraph{Shutdown of downstream units (Step 19)}
    \begin{itemize}
        \item There are 10 downstream process units with inventory.
