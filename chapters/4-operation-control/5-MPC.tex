...
Once a dynamic model is established, key controlled and manipulated variables will need to be set. While a comprehensive list of CVs and MVs is found in the plant wide survey in \ref{sec:PWS}, a summary table with key information is shown in Table \ref{tab:key-parameters}. A key parameter to control is the throughput manipulator (TPM, as discussed in \ref{ throughput to reactor R101, as this reactor produces the precursors for all 3 end products and hence affects the production capacity.  
