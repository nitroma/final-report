...
Process dynamics data can be utilised by the Relative Gain Array method to deduce the best manipulated variable pairing for each controlled variable. This is determined by the relative gain between a controlled variable (cv) and manipulated variable (mv), which indicates the ratio of the direct effect to the total effect of changing mv on cv (see Figure \ref{fig:RGA-block-di}). A good controller is one with the highest relative gain, because it means the direct effect dominates over the indirect effect which allows for the fastest controller response.
...
\begin{wrapfigure}{r}{0.5\linewidth}
    ...
    \caption{Block diagram showing the effect of changing mv1 on cv1. The direct effect of mv1 is shown via the pink path, while the indirect effect of mv1 is shown via the blue path. Image taken from \textcite{}}
