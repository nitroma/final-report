...
The established controls so far have assumed a single-input, single-output (SISO) system, where one manipulated variable only affects one control variable. In reality, it is likely that manipulating one variable will affect multiple controlled variables and we have a multiple-input, multiple-output (MIMO) system. For example, controlling reboiler duty would directly affect the column temperature, but would also indirectly affect the bottoms flow rate as more vapour is generated and less liquid is left. 

In principle, good control of MIMO systems will require knowledge about the dynamics of the system. This was beyond the scope of this report, however a more advanced controller design would be feasible once experimental work on a pilot scale plant was carried out to determine the dynamic responses of various manipulated variables to the controlled variables.

\subsubsection{Relative Gain Array}
Process dynamics data can be utilised by the Relative Gain Array method to deduce the best manipulated variable pairing for each controlled variable. This is determined by the relative gain between a controlled variable and manipulated variable, which is defined as the ratio of the steady state open-loop gain and closed-loop gain.
