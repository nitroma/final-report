...
The established controls so far have assumed a single-input, single-output (SISO) system, where one manipulated variable (mv) only affects one control variable (cv). In reality, it is likely that manipulating one variable will affect multiple controlled variables and we have a multiple-input, multiple-output (MIMO) system. For example, controlling reboiler duty would directly affect the column temperature, but would also indirectly affect the bottoms flow rate as more vapour is generated and less liquid is left. 
...
Process dynamics data can be utilised by the Relative Gain Array method to deduce the best manipulated variable pairing for each controlled variable. This is determined by the relative gain between a controlled variable and manipulated variable, which is defined as the ratio of the steady state open-loop gain and closed-loop gain (see Equation \ref{eqn:relative-gain}). This indicates the ratio of the direct effect to the total effect of changing mv on cv (see. A good controller is one with the highest relative gain, because it means the 
\begin{equation}
...
\begin{wrapfigure}{r}{0.6\linewidth}
    ...
    \caption{Block diagram showing the effect of changing }
    \label{fig:S203C-TC}
\end{wrapfigure}
