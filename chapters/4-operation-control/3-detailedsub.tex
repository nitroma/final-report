...
Ratio control is a special type of feedforward control whose objective is to maintain a specified ratio between two process streams. It is used in Nitroma's plant to maintain a stoichiometric ratio of reactants to reactor R201
...
The established controls so far have assumed that one manipulated variable (mv) only affects one control variable (cv). In reality, it is likely that manipulating one variable will affect multiple controlled variables and we have a multiple-input, multiple-output (MIMO) system. For example, reducing reboiler duty would directly affect the column temperature, but would also indirectly affect temperature thorothe pressure as less vapour is produced, which in turn . 

In principle, good control of MIMO systems will require knowledge about the dynamics of the system to choose the best mv-cv pairings and minimise interactions between control loops. This was beyond the scope of this report, however a more advanced controller design would be feasible once experimental work on a pilot scale plant was carried out to determine the dynamic responses of various mvs to the cvs.

\subsubsection{Relative Gain Array}
Process dynamics data can be utilised by the Relative Gain Array method to deduce the best manipulated variable pairing for each controlled variable. This is determined by the relative gain between a controlled variable and manipulated variable, which is defined as the ratio of the steady state open-loop gain and closed-loop gain (see Equation \ref{eqn:relative-gain}). This indicates the ratio of the direct effect to the total effect of changing mv on cv (see Figure \ref{fig:RGA-block-di}). A good controller is one with the highest relative gain, because it means the direct effect dominates over the indirect effect which allows for a fast controller response.
...
\begin{wrapfigure}{r}{0.5\linewidth}
    ...
    \caption{Block diagram showing the effect of changing mv1 on cv1. The direct effect of mv1 is shown via the pink path, while the indirect effect of mv1 is shown via the blue path.}
