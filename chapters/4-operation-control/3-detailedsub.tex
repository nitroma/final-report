...
The control loops discussed previously aim at maintaining optimum process conditions, however they might be insufficient to bring the system back to its set point following large and unexpected disturbances. To account for significant deviations from steady-state operation causing hazardous situations, additional control structures are put in place.  FirstlyTo ensure the plant is running at the desired operating conditions and to prevent hazardous events resulting from large disturbance, independent alarms for a range of variables such as temperature, pressure, level, O2 concentration, and H2 product quality are installed for all major units. Alarms also act as a layer of protection to ensure the risk is as low as reasonably practicable (ALARP) (see LOPA in Safety Section 3.6). When one or more alarms are triggered, the operators are notified, and manual actions need to be taken to bring the plant back to the desired operating condition. However, a large shock or disturbance might occur in a short timeframe and significantly increase the potential for hazardous events, and in this case alarms and manual action by the operators might not be sufficient to ensure the safety of the plant. Hence, emergency trips are designed and implemented to bring the plant into a safe state without any need for manual intervention when extremely hazardous conditions are detected in the plant and the risk of an incident is high.
...
\includegraphics[width=\linewidth]{chapters/4-operation-control/4-Figures/Alarms.pdf}
\end{table}
