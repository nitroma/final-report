...
Plant modularity refers to the degree of flexibility a process can accommodate. Nitroma will implement this process intensification concept in different aspects of its design. The plant is indeed multipurpose and can produce three valuable amino derivatives of toluene. The productivity of the process can be modified to align with 

%Another aspect of the modularity is that the productivity of the process may be altered to align with product demand uncertainties for diazomethane, and so allows for a great deal requirement for product variety39. While the chemical industry is often volatile towards speciality chemicals such as diazomethane, Diazome Tech. utilises the fact that diazomethane is a versatile reagent that is used in the production of a variety of different end-products to ensure modularity of its business strategy. This is because the process in which diazomethane is consumed in the methylation or acylation is selective and reacts readily under mild conditions. Fast time to market is realised due to the ability to carry out a safe and quick reconfiguration of the downstream process so that the end-product is not restricted to only alpha-halo ketones, should the product requirements change.


%Modularity refers to the degree of flexibility and variety a system can accommodate for and can be achieved by designing a greatly intensified process which generates a high purity product, allowing for the diazomethane to be used universally. Due to the small production scale, the potential for plant modularity can be realised through different aspects. In terms of modular construction, main process equipment can to be built from pre-assembled units and transported to the site with ease for full connection. Certain separation units such as the reverse osmosis and drying units are designed based on pre-specified, standardised “off-the-shelf” modules, which has the main advantage of reducing capital expenditure7.
