...
\resizebox{\textwidth}{!}{
\begin{tabular}{l|S[table-format=4.2]S[table-format=1.1]lS[table-format=7.0,round-mode=figures,round-precision=2]|ll|lll|S[table-format=1]S[table-format=1]c}
\toprule
                                          ...
                                          & {\splitcell{Price\\(\si{\USD\per\kg})}} & {\splitcell{CAGR\\(\%)}} & \rcell[30mm]{Average market share of producer (\%)} & {\splitcell{Demand\\(\si{\tonne\per\year})}} & \rcell{Number of reactions} & \rcell[25mm]{Max selectivity to toluene (\%)} & \rtext{Health} & \rtext{Flammability} & \rtext{Instability} & AHP & TOPSIS & Rank \\ \midrule
