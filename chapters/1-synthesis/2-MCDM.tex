...
On the contrary, criteria weightings and the associated inconsistency index can not be derived with TOPSIS alone []. The basis concept of the TOPSIS method is that the best alternative is the one which has the shortest distance to the ideal solution and longest distance to negative ideal solution []. The positive and negative ideal solutions are computed by respectively combining the best and worst performances for each criteria []. It is assumed that each criteria varies monotonically. The decision matrix of M alternatives and N criteria is normalised and the alternatives are ranked through comparing the Euclidean distances to the ideal best and worst solutions. This method allow for trade-off between criteria and distinguish between criteria that needed to be maximised and ones that needed to be minimised []. TOPSIS evaluation with criteria weightings derived from AHP is thus a suitable MDCM method for selecting the optimum products and synthesis routes.


\subsection{Sensitivity} %~0.5 page

Significant bias is introduced when deriving criteria weightings in AHP. The decision maker indeed subjectively determines the relative importance of one criteria over another, based on the companies

Sensi��vity analysis was conducted upon the criteria weigh��ngs to mi��gate any subjec��vity and bias
that could be introduced by AHP.

Two types of sensitivity analysis were conducted, both on the AHP weightage calculation. This was
chosen because values inputted into TOPSIS were all based on physical data and objective material
balance calculations. To check the overall optimality of the chosen option, a statistical analysis was
conducted where every possible combination of AHP paired ranking was tested.

Acute readers will realise that the weigh��ngs implemented in TOPSIS and AHP are subjec��ve and therefore
can vary widely depending on the individual. To explore how this could poten��ally affect decision
making, Monte Carlo simula��ons were performed to assess the sensi��vity of the weigh��ngs on the decisions
made. All the criteria weigh��ngs were simultaneously and randomly varied within a 50% range
where each simula��on ran for 4000 itera��ons.
However, as the process alters all criteria weigh��ngs simultaneously, the driving cause of a change in
rank and the alterna��ve preferred choice could not be determined explicitly. Sensi��vity coefficients (Ci)
were therefore calculated to evaluate the significance of varying weigh��ngs of each criterion on the decision
results.13 Both TOPSIS and AHP weigh��ngs were varied in 0.01 intervals un��l a change in the best
rank was observed.
...
\centering
   \begin{minipage}[c]{.46\linewidth}
      \includegraphics[width=\linewidth]{chapters/1-synthesis/1-Figures/AHP_product.pdf}
       \caption{Sensitivity of yield of Phthalic anhydride at optimum z=5.77m to changes in inlet feed T}
       \label{fig:AHP_product}
   \end{minipage} \hfill
   \begin{minipage}[c]{.46\linewidth}
      \includegraphics[width=\linewidth]{chapters/1-synthesis/1-Figures/TOPSIS_product.pdf}
      \caption{Sensitivity of maximum yield of Phthalic anhydride and optimum column height to changes in inlet feed T}
      \label{fig:TOPSIS_product}
   \end{minipage}
\end{figure}
