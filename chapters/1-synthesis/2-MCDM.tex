...
%AHP is a structured approach to analysing a complex problem and finding a unique solution that best suits the overall objective (Madurika & Hemakumara, 2015). First, the decision makers need to decompose their problem into a hierarchy of more comprehensive sub-problems, which are then analysed independently. The sub-problems, referred to as categories, are divided into different criteria called elements. The elements are compared pairwise and the relative importance of one element over another is determined by their impact to the category of the above hierarchy. AHP converts these evaluations to numerical values which can be processed over the entire problem (Saaty, 2008). Finally, numerical priorities are calculated for each option which represents the relative suitability of each alternative decision to achieve the objective of the problem.

%. The second method, AHP, involves pairwise comparisons between the categories listed in Table 1, followed by the pairwise comparison of criteria within each category to obtain a set of category weights and criterion weights. The category weight and criteria weights within the corresponding category were compounded to obtain the final weighting of each criteria (Saaty, 2008). The finalised weights are then used in combination with the criteria evaluation methods, with the overall preference between the alternatives being obtained by compounding the relative significance of each criterion and its weighting.

%On the other hand, according to Saaty and Vargas 41, AHP provides a simple yet systematic approach towards decision making scenarios with multiple criteria and options, and is not as data-intensive as MAUT. This is advantageous as it allows stakeholders to make decisions even with the lack of information in literature, especially with non-monetary criteria29. It decomposes complicated problems into simple hierarchies where elements in each hierarchy are compared pairwise and a final ranking is determined from the aggregated results of the pairwise comparisons. The relative preference of each element is determined regarding its impact in the next higher hierarchy and assigned a value for its intensity. Thus, it can provide evaluation scores for integrated quantitative analysis. However, this also means that it is not a suitable method to evaluate the different synthesis routes of diazomethane as it is unable to assess its strengths and weakness in isolation. Nonetheless, it remains very applicable in weight estimation for different criteria and group decision making49.

%AHP, developed by Saaty, compares the same criteria used in TOPSIS in a pairwise manner. Similar to TOPSIS, some criteria are more relevant than others, represented by a weighting from 1 to 9. The higher the weighting, the more a criterion dominates over another one. Once all of them had been compared, the weightings and scores were normalised and multiplied. AHP then recommends the choice with the highest sum of normalised scores. The weightings can also be checked for inconsistencies and contradictions. A consistency ratio can be calculated for each AHP method and if it is below 10 %, the weights are thus deemed consistent.12
