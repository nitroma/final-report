...
AHP decomposes complex problems into structure hierarchies to find a unique optimum solution meeting the overall objective []. The   

%. First, the decision makers need to decompose their problem into a hierarchy of more comprehensive sub-problems, which are then analysed independently. The sub-problems, referred to as categories, are divided into different criteria called elements. The elements are compared pairwise and the relative importance of one element over another is determined by their impact to the category of the above hierarchy. AHP converts these evaluations to numerical values which can be processed over the entire problem (Saaty, 2008). Finally, numerical priorities are calculated for each option which represents the relative suitability of each alternative decision to achieve the objective of the problem.
