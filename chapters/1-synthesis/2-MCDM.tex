...
% A variety of MCDM methods were initially considered: Simple Additive Weighting (SAW), Elimination and Choice Expressing Reality (ELECTRE), Complex Proportional Assessment (COPRAS), Analytic Network Process (ANP), Analytic Hierarchy Process (AHP), Technique for the Order Preference by Similarity to Ideal Solution (TOPSIS), and Weighted Aggregated Sum–Product Assessment (WASPAS). Since all methods have limitations, two methods will be used simultaneously to lower the risk of making an unfavourable decision. AHP and TOPSIS were selected due to their relative ease of usage with minimum subjectivity. Other methods were ruled out due to the lack of information for their inputs. Furthermore, some methods involve a single stakeholder making the final decision, and could be highly unreliable in the absence of other stakeholders’ perspectives (Zolfani & Zavadskas, 2013). 

%Within the recent few decades, numerous MCDM methods have been developed for different applications to tackle conflicting criteria and facilitate compromise in decision making. Each method is based on different theories and assumptions and has unique characteristics which allow them to be better suited for certain scenarios than others.

%In this report, several popular MCDM methods were considered: Weighted Sum method (WSM), Weighted product method (WPM), Analytical hierarchy process (AHP), Preference ranking organization method for enrichment evaluation (PROMETHEE), the elimination and choice translating reality (ELECTRE) and the technique for order preference by similarity to ideal solutions (TOPSIS), Multi-attribute utility theory (MAUT). The methods WSM and WPM do not account for the difference in units among criteria or combination of beneficial and non-beneficial criteria. ELECTRE has limited practical interest since it can only be applied when the criteria have identical ranges16. PROMETHEE is an advanced approach toward MCDM problem. However, due to the limited availability of precise quantitative data and the incomparability of units, the required input information of PROMETHEE cannot be satisfied49.

%On the other hand, according to Saaty and Vargas 41, AHP provides a simple yet systematic approach towards decision making scenarios with multiple criteria and options, and is not as data-intensive as MAUT. This is advantageous as it allows stakeholders to make decisions even with the lack of information in literature, especially with non-monetary criteria29

%Multi-criteria decision making (MCDM) has gained more and more importance in industry to aid decision making in a complex environment. Many methods have been developed and adopted, including Multi-Attribute Utility Theory (MAUT), ELimination and Choice Expressing REality (ELECTRE), Preference Ranking Organization METHod for Enrichment of Evaluations (PROMETHEE), Technique for Order of Preference by similarity to Ideal Solution (TOPSIS), Analytical Hierarchy Process (AHP) among others.8 It is widely acknowledged in literature that no one method is better than the others and a combination of methods should be used for a more accurate decision.9 AHP and TOPSIS were chosen by elimination after considering the advantages and disadvantages of each technique. MAUT was eliminated due to the large amount of input data required and the limited information this project has collected. Outranking methods, ELECTRE and PROMETHEE were also eliminated as the strength and weakness of alternatives were hard to determine with ELECTRE results and methods for assigning certain values in the PROMETHEE analysis were not clearly provided in literature.8 Disadvantages of TOPSIS and AHP however still exist and will be discussed later in this section.
...
\subsection{AHP and TOPSIS} %~0.5 page
