...
The inside battery limits (ISBL) investment is the cost of purchasing and installing all equipment. ISBL is calculated using various installation factors from Towler\cite{sinnott_chemical_2020}: 
\begin{equation}
    C=\sigma C_{e}*\left(\left(1+f_{p}\right)\frac{f_{er}+f_{i}+f_{c}+f_{s}+f_{l}}{f_{m}}\right)
\end{equation}
\subsection{Additional capital costs}
Values of each factor can found in Appendix.
